...
if __name__ == "__main__":
   ...
   ''' startServer(localPort)
    startElection(startUrl, startData)
    publicKey = waitForPublicKey()
    ...
    if(len(publicKey) > 0):    
        ...
        print("> Now lets encrypt votes")

        output, error = subprocess.Popen([node, "encrypt.js", json.dumps(publicKey).replace('"', '\"')], stdout = subprocess.PIPE).communicate()

        print("> Nodejs output is " + output)
        parsed = json.loads(output)
        ctexts = 'ctexts' + electionId
        print("> Writing file to " + ctexts)
        hash = writeVotes(parsed, ctexts)
        print("> Votes hash is " + hash)

        doTally(tallyUrl, tallyData, ctexts, hash)
        waitForTally()
    else:
        print("No public key, exiting..")    
    '''